...
function formSumbitted(event) {
	...
	const urlAlt = `https://api.dictionaryapi.dev/api/v2/entries/en_US/${inputFieldValue}`;

	// fetch data from Merriam-Webster Dictionary API (key required: obtained by registering & making an account at )
	...
	fetch(urlAlt)
		...
		.then(data => {
			...
			if (data[0].hwi == undefined) {
				...
				wordInput = data[0].hwi.hw;  // Free Dictionary: wordInput = data[0].word;
				partOfSpeech = data[0].fl;	 //	partOfSpeech = data[0].meanings[0].partOfSpeech;
			}
		});
		...
		if (nouns.length == 3 && adjs.length == 2 && adv != "") {
			...
			$container.innerHTML = "Jessie and her best friend Dave went to Disney World today! They saw a " + nouns[0] + " in a show at the Magic Kingdom and ate a " + adjs[0] + " feast for dinner. The next day I ran " + adv + " to meet Mickey Mouse in his " + nouns[1] + " and then that night I gazed at the 1000 " + adjs[1] + " fireworks shooting from the " + nouns[2] + ".";
		}
	$inputField.value = "";
